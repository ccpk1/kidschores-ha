...
def async_setup_services(hass: HomeAssistant):
    ...
    async def handle_claim_chore(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Claim Chore: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_approve_chore(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Approve Chore: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_redeem_reward(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Redeem Reward: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_apply_penalty(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Apply Penalty: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_reset_bonuses(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Reset Bonuses: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_reset_rewards(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Reset Rewards: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_remove_awarded_badges(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Remove Awarded Badges: %s",
                const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND,
            )
            return
    ...
    async def handle_apply_bonus(call: ServiceCall):
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Apply Bonus: %s", const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND
            )
            return
    ...
    async def handle_reset_overdue_chores(call: ServiceCall) -> None:
        ...
        if not entry_id:
            const.LOGGER.warning(
                "WARNING: Reset Overdue Chores: %s",
                const.TRANS_KEY_ERROR_MSG_NO_ENTRY_FOUND,
            )
            return
