...
class KidScheduleCalendar(CalendarEntity):
    ...
    def __init__(self, coordinator, kid_id: str, kid_name: str, config_entry, calendar_duration):
        ...
        self._attr_device_info = kh.create_kid_device_info(kid_id, kid_name, config_entry)
    ...
    def _generate_events_for_chore(
        ...
        summary = chore.get(const.DATA_CHORE_NAME, const.TRANS_KEY_DISPLAY_UNKNOWN_CHORE)
        description = chore.get(const.DATA_CHORE_DESCRIPTION, const.SENTINEL_EMPTY)
        recurring = chore.get(const.DATA_CHORE_RECURRING_FREQUENCY, const.FREQUENCY_NONE)
        ...
        completion_criteria = chore.get(const.DATA_CHORE_COMPLETION_CRITERIA, const.SENTINEL_EMPTY)
        ...
        if due_dt:
            ...
            if recurring == const.FREQUENCY_DAILY:
                ...
                unit = chore.get(const.DATA_CHORE_CUSTOM_INTERVAL_UNIT, const.TIME_UNIT_DAYS)
        ...
        gen_start = window_start
        future_limit = dt_util.as_local(datetime.datetime.now() + self._calendar_duration)
    ...
    def _generate_events_for_challenge(
        ...
        description = challenge.get(const.DATA_CHALLENGE_DESCRIPTION, const.SENTINEL_EMPTY)
    ...
    def _generate_all_events(
        ...
        for chore in self.coordinator.chores_data.values():
            if self._kid_id in chore.get(const.DATA_CHORE_ASSIGNED_KIDS, []):
                events.extend(self._generate_events_for_chore(chore, window_start, window_end))
        # challenges
        for challenge in self.coordinator.challenges_data.values():
            if self._kid_id in challenge.get(const.DATA_CHALLENGE_ASSIGNED_KIDS, []):
                events.extend(
                    self._generate_events_for_challenge(challenge, window_start, window_end)
                )
        return events
