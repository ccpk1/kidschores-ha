...
class RecurrenceEngine:
    ...
    def _calculate_with_rrule(
        ...
        """
        freq = self._frequency
        base_local = dt_util.as_local(self._base_date) if self._base_date else None
        if not base_local:
            return None
        ...
        reference_local = dt_util.as_local(reference_utc)
        next_occurrence = rule.after(reference_local, inc=not require_future)

        if next_occurrence:
            return dt_util.as_utc(next_occurrence)
        return None
    ...
    def _get_day_end(self, dt: datetime) -> datetime:
        ...
        """
        start_of_day = dt_util.start_of_local_day(dt)
    ...
    def _get_week_end(self, dt: datetime) -> datetime:
        ...
        start_of_sunday = dt_util.start_of_local_day(sunday)
    ...
    def _get_month_end(self, dt: datetime) -> datetime:
        ...
        start_of_last_day = dt_util.start_of_local_day(month_end_date)
    ...
    def _get_quarter_end(self, dt: datetime) -> datetime:
        ...
        start_of_last_day = dt_util.start_of_local_day(quarter_end_date)
        return start_of_last_day + timedelta(
            hours=const.END_OF_DAY_HOUR,
            minutes=const.END_OF_DAY_MINUTE,
            seconds=const.END_OF_DAY_SECOND,
    ...
    def _get_year_end(self, dt: datetime) -> datetime:
        ...
        start_of_last_day = dt_util.start_of_local_day(year_end_date)
        return start_of_last_day + timedelta(
            hours=const.END_OF_DAY_HOUR,
            minutes=const.END_OF_DAY_MINUTE,
            seconds=const.END_OF_DAY_SECOND,
...
def add_interval(
    ...
    if isinstance(base_date, datetime):
        base_dt = dt_util.as_utc(base_date)
    else:
        try:
            parsed = dt_util.parse_datetime(base_date)
            if not parsed:
                const.LOGGER.error(
                    "add_interval: Could not parse base_date: %s", base_date
                )
                return None
            base_dt = dt_util.as_utc(parsed)
    ...
    if require_future:
        ref_utc = reference_datetime or dt_util.utcnow()
        if not isinstance(ref_utc, datetime):
            ref_utc = dt_util.utcnow()
        ref_utc = dt_util.as_utc(ref_utc)
...
def _apply_period_end(dt: datetime, period: str) -> datetime:
    ...
    if period == const.PERIOD_DAY_END:
        start = dt_util.start_of_local_day(local_dt)
    ...
    if period == const.PERIOD_WEEK_END:
        ...
        start = dt_util.start_of_local_day(sunday)
        return start + timedelta(
            hours=const.END_OF_DAY_HOUR,
            minutes=const.END_OF_DAY_MINUTE,
            seconds=const.END_OF_DAY_SECOND,
    ...
    if period == const.PERIOD_MONTH_END:
        ...
        start = dt_util.start_of_local_day(month_end)
        return start + timedelta(
            hours=const.END_OF_DAY_HOUR,
            minutes=const.END_OF_DAY_MINUTE,
            seconds=const.END_OF_DAY_SECOND,
    ...
    if period == const.PERIOD_QUARTER_END:
        ...
        start = dt_util.start_of_local_day(quarter_end)
        return start + timedelta(
            hours=const.END_OF_DAY_HOUR,
            minutes=const.END_OF_DAY_MINUTE,
            seconds=const.END_OF_DAY_SECOND,
    ...
    if period == const.PERIOD_YEAR_END:
        ...
        start = dt_util.start_of_local_day(year_end)
        return start + timedelta(
            hours=const.END_OF_DAY_HOUR,
            minutes=const.END_OF_DAY_MINUTE,
            seconds=const.END_OF_DAY_SECOND,
