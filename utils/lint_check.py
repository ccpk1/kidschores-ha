    ...
    try:
        ...
        if "generalDiagnostics" in data:
            for diag in data["generalDiagnostics"]:
                ...
                if file_path not in diag_file and not diag_file.endswith(file_path.split("/")[-1]):
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Comprehensive linting check for KidsChores")
    parser.add_argument(
        "--integration",
        action="store_true",
