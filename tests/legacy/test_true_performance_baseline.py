...
async def test_stress_dataset_true_performance(
    ...
    try:
        ...
        if perf_summary:
            for op, stats in perf_summary.items():
                ...
                if "check_badges_for_kid" in op and len(coordinator.kids_data) > 0:
                    ...
                    operations = len(coordinator.chores_data) * len(coordinator.kids_data)
                    if operations > 0:
        ...
        speedup = sequential_per_kid / concurrent_per_kid if concurrent_per_kid > 0 else 0
        if speedup > 0:
            pass
    ...
    try:
        ...
        with (
            ...
            for _ in range(10):
                overdue_start = time.perf_counter()
                await coordinator._check_overdue_chores()
                overdue_duration = (time.perf_counter() - overdue_start) * 1000  # milliseconds
            ...
            badge_duration_ms = (time.perf_counter() - badge_start) * 1000  # milliseconds
            badge_duration_ms / len(coordinator.kids_data)
            (badge_duration_ms * 1000) / (len(coordinator.kids_data) * len(coordinator.badges_data))

            # Test data persistence (measure queue time precisely)
            persist_start = time.perf_counter()
            coordinator._persist()  # Not async
            persist_queue_us = (time.perf_counter() - persist_start) * 1000000  # microseconds
