...
async def test_specific_friendly_names(hass: HomeAssistant, scenario_medium) -> None:
    ...
    assert "UI Dashboard Helper" in dashboard_helper.attributes.get("friendly_name", ""), (
    ...
    assert "UI Dashboard Date Helper" in date_helper.attributes.get("friendly_name", ""), (
...
async def test_chore_status_friendly_name(hass: HomeAssistant, scenario_medium) -> None:
    ...
    chore_status_sensors = [
        entry
        for entry in entries
        if entry.entity_id.startswith("sensor.kc_") and "_chore_status_" in entry.entity_id
    ]
