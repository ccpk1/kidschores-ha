...
| Phase 4 – Gamification Toggle | Skip points/badges when disabled | 50% | ⚡ Core done, sensor filtering pending |
  ...
  3. **Deletion cascade** (Status: Not started)
     ...
     **Implementation in coordinator.py** `delete_parent_entity()`:
     ...
     def delete_parent_entity(self, parent_id: str) -> None:
         """Delete a parent entity and cascade to shadow kid if exists."""
         ...
         if shadow_kid_id and shadow_kid_id in self._data[const.DATA_KIDS]:
             const.LOGGER.info(
                 "Cascade deleting shadow kid '%s' for parent '%s'",
