...
- **Status**: In progress (implementation complete, awaiting owner archive approval)
...
3. **Next steps (short term)**
   - Request owner approval to archive the plan and move artifacts to `docs/completed/`.
   - Optionally run one additional HA runtime smoke loop if you want post-fix warning-free evidence in a fresh log window.
   - Prepare final commit/PR summary for maintainers.
4. **Risks / blockers**
...
5. **References** –
   ...
   - `docs/in-process/ASYNC_LISTENER_MIGRATION_SUP_IMPLEMENTATION_MATRIX.md`
