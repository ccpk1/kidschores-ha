...
6. **Decisions & completion check**
   - **Decisions captured**:
     - [ ] Use Strategy Pattern with criterion handler registry (not if/else chains)
     - [ ] Debounce manager evaluation (2s window) to prevent redundant calculations
     - [ ] Separate acquisition from retention logic for testability
     - [ ] Engine returns result objects; Manager handles notifications
